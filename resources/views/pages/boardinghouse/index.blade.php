...
@section('content')
    ...
    <div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
        <div class="flex flex-col gap-[6px]">
            ...
            <p class="text-ngekos-grey">Tersedia {{ $results->count() }} Kos</p>
        </div>
    </div>
            ...
            <a href="details.html" class="card">
                <div
                    ...
                    <div class="flex w-[120px] h-[183px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                        <img src="{{ asset('storage/' . $result->thumbnail) }}" class="w-full h-full object-cover"
                            alt="icon">
                    </div>
                    <div class="flex flex-col gap-3 w-full">
                        <h3 class="font-semibold text-lg leading-[27px] line-clamp-2 min-h-[54px]">{{ $result->name }}</h3>
                        <hr class="border-[#F1F2F6]">
                        <div class="flex items-center gap-[6px]">
                            <img src="{{ asset('assets/images/icons/location.svg') }}" class="w-5 h-5 flex shrink-0"
                                alt="icon">
                            <p class="text-sm text-ngekos-grey">{{ $result->city->name }}</p>
                        </div>
                        <div class="flex items-center gap-[6px]">
                        ...
                        <p class="font-semibold text-lg text-ngekos-orange">Rp
                            {{ number_format($result->price, 0, ',', '.') }}<span
