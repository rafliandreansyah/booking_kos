...
@section('content')
    ...
    <form action="{{ route('information.save', $boardingHouse->slug) }}"
...
@section('scripts')
    <script>
        const defaultPrice = {{ $room->price_per_month ?? 0 }};
    </script>

    <script src="{{ asset('assets/js/cust-info.js') }}" type="text/javascript" cache-control="no-cache"></script>
@endsection
